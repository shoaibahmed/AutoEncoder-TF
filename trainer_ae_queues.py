...
parser.add_option("--batchSize", action="store", type="int", dest="batchSize", default=200, help="Batch size")
...
# Directories
parser.add_option("--logsDir", action="store", type="string", dest="logsDir", default="/netscratch/siddiqui/Bosch/src/logs-32", help="Directory for saving logs")
parser.add_option("--modelDir", action="store", type="string", dest="modelDir", default="/netscratch/siddiqui/Bosch/src/bosch-autoencoder-32/", help="Directory for saving the model")
...
parser.add_option("--imagesOutputDirectory", action="store", type="string", dest="imagesOutputDirectory", default="/netscratch/siddiqui/Bosch/src/outputImages-32", help="Directory for saving output images")
...
def preprocess_image(img):
	...
	imgMean, imgVar = tf.nn.moments(img, axes=[0])
	img = (img - imgMean) / imgVar
	return img

def load_image(filename_queue):
	reader = tf.WholeFileReader()
	key, image_content = reader.read(filename_queue)
	# image_content = tf.read_file(filename_queue)

	image = tf.image.decode_jpeg(image_content, channels=options.imageChannels) # use png or jpg decoder based on your files.
	# Pre-process images
	image = preprocess_image(image)
	return image

def input_pipeline(images_dir, batch_size, num_epochs):
	# filename_queue = tf.train.string_input_producer(filenames, num_epochs=num_epochs, shuffle=True)
	filename_queue = tf.train.string_input_producer(tf.train.match_filenames_once(images_dir))
	example = load_image(filename_queue)
	...
	example_batch = tf.train.shuffle_batch([example], batch_size=batch_size, capacity=capacity, min_after_dequeue=min_after_dequeue, num_threads=6)
...
if options.trainModel:
	with tf.variable_scope('Model'):
		if options.convolutionalAutoencoder:
			...
			# with open(options.trainFileName) as f:
			# 	trainFileNames = f.readlines()
			# trainFileNames = [x.strip() for x in trainFileNames] 
			print ("Creating input pipeline")
			# inputBatchImages = input_pipeline(filenames=trainFileNames, batch_size=options.batchSize, num_epochs=options.trainingEpochs)
			inputBatchImages = input_pipeline(images_dir="/netscratch/schulze/bosch/Cam3_iO/*.jpg", batch_size=options.batchSize, num_epochs=options.trainingEpochs)
			print (inputBatchImages.get_shape())
...
if options.trainModel:
	with tf.Session() as sess:
		...
		try:
			...
			print('Done training for %d epochs, %d steps.' % (options.trainingEpochs, step))
		finally:
